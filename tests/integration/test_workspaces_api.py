...
async def test_workspaces_api_create_select_and_validation(async_client):
    ...
    deleted = await async_client.delete(f"/api/workspaces/{created_payload['id']}")
    assert deleted.status_code == 204

    listed_after_delete = await async_client.get("/api/workspaces")
    assert listed_after_delete.status_code == 200
    listed_after_delete_entries = listed_after_delete.json()["entries"]
    assert len(listed_after_delete_entries) == 1
    assert listed_after_delete_entries[0]["id"] == default_workspace["id"]

    active_delete = await async_client.delete(f"/api/workspaces/{default_workspace['id']}")
    assert active_delete.status_code == 409
    assert active_delete.json()["error"]["code"] == "workspace_active_delete_forbidden"
    ...
    missing_delete = await async_client.delete("/api/workspaces/missing")
    assert missing_delete.status_code == 404
    assert missing_delete.json()["error"]["code"] == "workspace_not_found"
