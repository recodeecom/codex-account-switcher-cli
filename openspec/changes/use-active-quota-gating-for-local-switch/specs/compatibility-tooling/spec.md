...
#### Scenario: Use this action is enabled for active accounts with 5h quota

- **WHEN** an account status resolves to `active`
- **AND** `primary_remaining_percent > 0`
...
- **WHEN** an account is active with positive 5h quota but no matched codex-auth snapshot exists
- **THEN** the UI still allows clicking **Use this**
- **AND** the switch request can return a snapshot-not-found error for user remediation
